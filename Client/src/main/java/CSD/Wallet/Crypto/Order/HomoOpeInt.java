...
public class HomoOpeInt {
	...
	OpeDomainRange lazySample(long dLow, long dHigh, long rLow, long rHigh, long valor, boolean cifra) {
		...
		if (ci == null) {
			...
			dGap = ci.longValue();
		boolean goLow = true;
		if (cifra && valor >= dLow + dGap)
			goLow = false;
		if (!cifra && valor >= rLow + rGap)
			goLow = false;
		if (goLow)
	...
	// Fun��es adaptadas de
	// https://github.com/masih/sina/blob/master/src/main/java/DistLib/hypergeometric.java
	...
	public double hgd(long kk, long nn1, long nn2, Random rand) {
		...
		if (minjx == maxjx) {
			...
			if (setup1 || setup2) {
				...
				double conta = a - afc((long) (xl)) - afc((long) (n1 - xl)) - afc((long) (k - xl))
						- afc((long) (n2 - k + xl));
				// System.out.println("Conta: "+conta);
				if (conta > 5)
					conta = 5;// limita��o da exponencial
				kl = Math.exp(conta);
				/* !* kr = exp(a - afc((int) (xr - 1)) *! */
				conta = a - afc((long) (xr - 1)) - afc((long) (n1 - xr + 1)) - afc((long) (k - xr + 1))
						- afc((long) (n2 - k + xr - 1));
				// System.out.println("Conta: "+conta);
				if (conta > 5)
					conta = 5;// limita��o da exponencial
				...
				p2 = p1 + kl / lamdl;
				p3 = p2 + kr / lamdr;
				// System.out.println("a "+a+" kl "+kl+" kr "+kr+" lamdl "+lamdl+" lamdr
				// "+lamdr);
				// System.out.println("O p1 e "+p1+" o p2 e "+p2+" o p3 e "+p3);
			}
			// System.out.println("H2PE, cheguei ao while, p3 "+p3+" p2 "+p2+" kl "+kl+" kr
			// "+kr);
			L30: while (true) {
